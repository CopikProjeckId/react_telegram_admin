...
import DataCall, { useTugDashBoard } from "../../../lib/module/data.call";
...
export default function TugUserList() {
  ...
  const dashBoard = useTugDashBoard((state) => state);
  ...
  useEffect(() => {
    DataCall.getInstance().getUserList("tug");
  }, []);
  ...
  const dataSort = (key: UserListEnum, type: "asc" | "desc") => {
    ...
    if (key === UserListEnum.ITEM_LV) {
      if (type === "asc") {
        data = data?.sort((a, b) => {
          return (a as any).user_item_lv - (b as any).user_item_lv;
        });
      } else {
        data = data?.sort((a, b) => {
          return (b as any).user_item_lv - (a as any).user_item_lv;
        });
      }
    }
  ...
  const getUserDetail = async (id: string) => {
    DataCall.getInstance()
      .getUserDetail(id, "tug")
      .then((res) => {
        setUser_detail(res);
        setEdit_item(res.user_item.map((item: any) => item.lv));
