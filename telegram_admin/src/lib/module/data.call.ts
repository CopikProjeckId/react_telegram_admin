...
export default class DataCall {
	...
	public async get_dashboard_data(type?: 'tug' | 'red_and_green') {
		if (type == 'tug') {
	...
	public async getLiveGameCount(type?: 'tug' | 'red_and_green') {
		if (type == 'tug') {
	...
	public async getUserList(type?: 'tug' | 'red_and_green') {
		if (type == 'tug') {
	...
	public async getUserDetail(id: string, type?: 'tug' | 'red_and_green') {
		if (type == 'tug') {
	...
	public async editUserItem(user_id: string, id: string, count: number, type?: 'tug' | 'red_and_green') {
		if (type == 'tug') {
			return await this.fetch._put("admin/user-item", { telegramUserId: user_id, id: id, count: count }, APP_CONFIG.APP_TUG_URL).then((res) => {
				return res.data.user;
			});
		} else {
	...
	public async editUserToken(user_id: string, token: string, value: number, type?: 'tug' | 'red_and_green') {
		if (type == 'tug') {
	...
	public async getItemList(type?: 'tug' | 'red_and_green') {
		if (type == 'tug') {
	...
	public async updateItem(item: DB_TYPE["item"], type?: 'tug' | 'red_and_green') {
		if (type == 'tug') {
			let item_data = {
				price: item.price,
	...
	public async getMissionList(type?: 'tug' | 'red_and_green') {
		if (type == 'tug') {
	...
	public async updateMission(mission: DB_TYPE["bounsmissiontable"], type?: 'tug' | 'red_and_green') {
		if (type == 'tug') {
	...
	public async createItem(item: DB_TYPE["item"], type?: 'tug' | 'red_and_green') {
		if (type == 'tug') {
	...
	public async createMission(mission: DB_TYPE["bounsmissiontable"], type?: 'tug' | 'red_and_green') {
		if (type == 'tug') {
	...
	public async getConfigChange(type?: 'tug' | 'red_and_green') {
		if (type == 'tug') {
	...
	public async getConfigChangeItem(type?: 'tug' | 'red_and_green') {
		if (type == 'tug') {
	...
	public async updateConfigChange(config: any, type?: 'tug' | 'red_and_green') {
		if (type == 'tug') {
	...
	public async updateConfigChangeItem(config: any, type?: 'tug' | 'red_and_green') {
		if (type == 'tug') {
	...
	public async getUserListForMessage(type?: 'tug' | 'red_and_green') {
		if (type == 'tug') {
	...
	public async sendMessageToUser(user_id: string | string[], message: string, message_type: string, type?: 'tug' | 'red_and_green') {
		let data = {
			user_id: user_id,
			message: message,
			message_type: message_type
		};
		await this.fetch._post("admin/send-message-to-user", data, type == 'tug' ? APP_CONFIG.APP_TUG_URL : APP_CONFIG.APP_FETCH_URL).then((res) => {
			return res;
		});
	}
	public async getCallGameData({ id, start, limit }: { id: string, start?: number, limit?: number }) {
		return await this.fetch._get("admin/game-data", {
			id: id,
			start: start,
			limit: limit
		}, APP_CONFIG.APP_FETCH_URL).then((res) => {
			return res;
		});
	}
	public async getNotificationList() {
		return await this.fetch._get("admin/notification", {}, APP_CONFIG.APP_FETCH_URL).then((res) => {
			return res;
		});
	}
	public async updateNotifications(notification: any) {
		return await this.fetch._put("admin/notification", { ...notification }, APP_CONFIG.APP_FETCH_URL).then((res) => {
			return res;
		});
	}
	public async createNotification(notification: any) {
		let formData = new FormData();
		formData.append("file", notification.thumbnail);
		formData.append("notification", JSON.stringify(notification));
		return await this.fetch._post_form_data("admin/notification", formData, APP_CONFIG.APP_FETCH_URL).then((res) => {
			return res;
		});
	}
}
